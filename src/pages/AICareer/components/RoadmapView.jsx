  ...
  return (
    <div className="space-y-12 max-w-5xl mx-auto">
      ...
      <Card className="p-0 overflow-hidden border-none shadow-2xl">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-10 md:p-14 text-white relative">
          ...
          <div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex justify-between items-center p-5 rounded-2xl bg-slate-800/40 border border-slate-700/50">
                ...
                <span className="font-bold text-2xl text-blue-400">{roadmap.careerInsights?.demand || '—'}</span>
              </div>
            </div>
          </div>
            ...
            <div className="h-44 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={skillData} layout="vertical" margin={{ left: -10, right: 30 }}>
                  ...
                  <Tooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '12px', fontSize: '12px' }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <button
                ...
                className={`w-full text-left p-5 rounded-2xl border transition-all relative flex flex-col ${
                  activePhaseIndex === idx
                    ? 'border-blue-500 bg-blue-500/10 shadow-xl shadow-blue-500/10 ring-1 ring-blue-500/20'
                    : 'border-slate-800 hover:border-slate-700 bg-slate-900/50'
                }`}
              >
                <div className={`absolute left-[-2.25rem] top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-4 z-20 transition-all duration-300 ${activePhaseIndex === idx ? 'bg-blue-500 border-slate-900 scale-125' : 'bg-slate-700 border-slate-900'}`} />
                <div className={`text-[10px] font-black uppercase tracking-widest mb-1 ${activePhaseIndex === idx ? 'text-blue-400' : 'text-slate-500'}`}>
        ...
        <div className="lg:col-span-8">
          {phase && (
            <Card className="min-h-[400px] border-t-8 border-t-blue-500 p-10 md:p-14">
              <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10">
                <div className="space-y-2">
                  <span className="inline-block px-3 py-1 rounded-lg bg-blue-500/10 text-blue-400 text-xs font-bold tracking-widest uppercase">Stage Details</span>
                ...
                <div className="flex flex-wrap gap-4">
                  {(phase.resources || []).map((resource, i) => (
                    <button
                      ...
                      className="px-6 py-3 rounded-2xl bg-slate-800/60 hover:bg-slate-700 border border-slate-700 hover:border-blue-500/50 transition-all text-sm font-bold flex items-center gap-3 text-left"
                    >
