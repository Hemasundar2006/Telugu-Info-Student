...
export default function HallTickets() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const hallTickets = (res?.data ?? []).filter((d) => d.docType === 'HALL_TICKET');
        setDocs(hallTickets);
      } catch (err) {
  ...
  return (
    <div className="documents-page">
      <h1>
        <FiFileText className="doc-section-icon" size={24} style={{ marginRight: '0.5rem', verticalAlign: 'middle' }} />
        Hall Tickets
      </h1>
      <p className="section-desc">View and download any available hall ticket here.</p>
      ...
      {filteredDocs.length === 0 ? (
        <p className="doc-section-empty">No hall tickets available yet.</p>
      ) : (
