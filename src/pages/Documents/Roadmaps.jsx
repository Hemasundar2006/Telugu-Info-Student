...
export default function Roadmaps() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const roadmaps = (res?.data ?? []).filter((d) => d.docType === 'ROADMAP');
        setDocs(roadmaps);
      } catch (err) {
  ...
  return (
    <div className="documents-page">
      <h1>
        <FiMap className="doc-section-icon" size={24} style={{ marginRight: '0.5rem', verticalAlign: 'middle' }} />
        AI Roadmaps
      </h1>
      <p className="section-desc">View and download AI-driven career roadmaps and guidance here.</p>
