...
export default function SupportAdminChat() {
  ...
  const loadMessages = async () => {
    setError('');
    try {
      const res = await getSupportAdminMessages();
  ...
  const handleSend = async (e) => {
    ...
    setError('');
    try {
      const res = await sendSupportAdminMessage({
  ...
  return (
    <div className="chat-page">
      <div className="chat-header">
        <h1>Support ↔ Admin Chat</h1>
        <p>Internal channel between support and admin teams.</p>
      </div>
